function export_shape_ply(fshape,path,format)
% EXPORT_PLY(fshape) save a fshape structure in .ply. (This function uses a function written by P. Getreuer)
%
%Input :
%   fshape: struct('x',...,'G',...,'f',...)
%   path : string 
%   format (optional) : 'bin' (bianary file, default) or 'ascii'
%

if size(fshape.x,2)<=2
    fshape.x = [fshape.x,zeros(size(fshape.x,1),3-size(fshape.x,2))];
end 
...
data.vertex = struct('x',double(fshape.x(:,1)),'y',double(fshape.x(:,2)),'z',double(fshape.x(:,3)));
data.face = struct('vertex_indices',{mat2cell(fshape.G -1,ones(1,size(fshape.G,1)),size(fshape.G,2))'});

ply_write(data,path,format);
