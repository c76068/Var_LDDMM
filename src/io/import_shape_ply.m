function fshape = import_shape_ply(path)
% IMPORT_PLY(path) import a shape or fshape from a ply file. (This function uses a function written by P. Getreuer)
%
% The signal fshape.f of fshape is equal to the mean of the RBG signal of the .ply file. 
%
%Input :
...
%   fshape : struct('x',...,'G',...,'f',...)
%
% See also : import_shape_vtk, import_shape_obj
%

[~,~,s]= ply_read(path,'tri');


if isfield(s.vertex,'red') && isfield(s.vertex,'green') && isfield(s.vertex,'blue')
    fshape = struct('x',[s.vertex.x,s.vertex.y,s.vertex.z],'G',(cell2mat(s.face.vertex_indices)' + 1)','f',mean([s.vertex.red,s.vertex.blue,s.vertex.green],2));
else
    fshape = struct('x',[s.vertex.x,s.vertex.y,s.vertex.z],'G',(cell2mat(s.face.vertex_indices)' + 1)');
end
